...
# 基于 DKU 2025-2026 bulletin 的 major 课程集合（按 major/track 归档）
# 说明：
# 1. 这里的集合主要用于 recommendation priority 计算，因此把 major requirement 中
#    出现的必修/限选课程都视为“专业相关课程”。
# 2. 真正的毕业审核仍然建议继续以 request.graduation_requirements 为准。
...
def save_recommendation_history(student_id: str, recommendations: List[Dict]):
    ...
    data["recommendation_history"].append(history_entry)
...
class RecommendationEngine:
    ...
    def is_major_related_course(course: CourseInput, profile: StudentProfile, requirements: GraduationRequirement) -> bool:
        ...
        return (
            ...
            or course.code in requirements.required_courses
            or course.code in major_course_set
        )
    ...
    def calculate_priority_score(
        ...
        # 只要课程出现在该专业的 requirement 中，就给 major 优先级
        if is_major_related:
            score += 0.45

        if course.code in requirements.required_courses:
            score += 0.15
    ...
    def get_recommendation_reason(
        ...
        if course.core_requirement:
            reasons.append("核心必修课程")
        if is_major_related:
            reasons.append("属于当前专业要求课程")
        if course.code in requirements.required_courses:
            reasons.append("毕业要求课程")

        missing_prereqs = [p for p in course.prerequisites if p not in profile.completed_courses]
        if not missing_prereqs:
            reasons.append("先修课程已满足")
        else:
            reasons.append(f"缺少先修课程: {', '.join(missing_prereqs)}")

        if profile.current_credits < requirements.total_credits * 0.5:
            reasons.append("适合当前学分阶段")
        elif profile.current_credits < requirements.total_credits * 0.8:
            reasons.append("适合作为中期选课安排")
        else:
            reasons.append("适合毕业前阶段补足要求")

        return "；".join(reasons) if reasons else "一般推荐课程"
    ...
    def get_risk_warning(course: CourseInput) -> Optional[str]:
        ...
        if enrollment_ratio > 0.9:
            warnings.append("课程名额紧张")
        elif enrollment_ratio > 0.7:
            warnings.append("课程名额有限")

        if course.difficulty >= 4.0:
            warnings.append("课程难度较高")
        elif course.difficulty >= 3.5:
            warnings.append("课程有一定难度")
